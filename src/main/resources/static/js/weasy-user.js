	...
	$(".loginBtn").on('click',
		...
		function signUpSubmit() {
			...
			if(nameVal.val() == "") {
	         ...
	         emailWarning.html("이메일 형식이 올바르지 않습니다.").css('color', 'red');;
	         emailVal.focus();
	         return false;
	         
	      	} else if(checkEmailFail == undefined || checkEmailFail == 1){ //이메일 중복검사 필수로 설정
			  emailWarning.html("이메일 중복확인이 필요합니다.").css('color', 'red');;
			  emailVal.focus();
			  return false;
	      	...
	      	if (pwVal.val() == "") {
	         	...
	         	pwWarning.html("8~16자 영문 대 소문자, 숫자, 특수문자를 사용해주세요.");
	         	pwVal.focus();
	         	return false;
	        ...
	        if(!userBirth.test(birthVal.val())) {
	      	...
	      	/*닉네임 공백 및 유효성 확인*/
	      	if (nickNameVal.val() == "") {
	         	nicknameWarning.html("닉네임을 입력해주세요");
	         	nickNameVal.focus();
	         	return false;
